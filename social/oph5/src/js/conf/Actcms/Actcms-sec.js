define("conf/Actcms/Actcms-sec.js",function (require,exports,module){
		    ...
		    if (data.success) {
				if(data.data.list.length == 0){
					...
					for(var i=0;i<oData.length;i++){
     					var addSection = 										
							'<li class="fl section5-cont1-li section5-cont1-liadd" id="oS5ContLiL">'+									
	         					'<div class="section5-cont-li-div" shopId="'+oData[i].shopId+'" productId="'+oData[i].productId+'" sn="'+oData[i].sn+'" icon="'+data.icon+'">'+
	         						'<a href="javascript:;" producturl="'+domain+'?shopId='+oData[i].shopId+'&productId='+oData[i].productId+'">'+
	         							'<img src="'+oData[i].productPicUrl+'" alt="国美Plus">'+
	         						...
	         						'</div>'+
	         						'</a>'+
	         						'<a href="javascript:;" producturl="'+domain+'?shopId='+oData[i].shopId+'&productId='+oData[i].productId+'" class="couponsa" style="color:#ee2f2f;">'+
	         							'<div class="coupons">'+
	...
	toBpsendUrl($('.oSection9 a'),'Mhd0A000','活动位');
	toBpsendUrl($('.oSection0 a'),'Mhd0A001','H5-模块1');
	$('.actcms-section a').click(function(){
		var productId = $(this).attr('productid');
		var shopId = $(this).attr('shopid');
	...
	$('body').on('click','.section5-cont1-liadd a',function(){
		var productId = $(this).attr('productid');
		var shopId = $(this).attr('shopid');
		var productUrl = $(this).attr('producturl');
		if(productUrl){
