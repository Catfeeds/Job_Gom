...
define('conf/staff/info-bind-new.js', function (require) {
    var $ = require('$');
    ...
    if(window.isOverTime != 1){
        ...
        $('#first-dep').click(function(){
            ...
            typeOfInputVal = 'firstDep';
                ...
                $.ajax({
                    type:'get',
                    url:location.origin+'/op/shopbind/getchild',
                    data:{'dept_id':$('#first-dep-inp').attr('parent-id')},
                    dataType:'json',
                    async:true,
        ...
        $('#second-dep').click(function(){
            ...
            typeOfInputVal = 'secondDep';
                ...
                $.ajax({
                    type:'get',
                    url:location.origin+'/op/shopbind/getchild',
                    data:{'dept_id':$('#second-dep-inp').attr('parent-id')},
                    dataType:'json',
                    async:true,
                    success:function(res){
        ...
        $('#companyname-icon').click(function(){
            ...
            typeOfInputVal = 'companyName';
                ...
                $.ajax({
                    type:'get',
                    url:location.origin+'/op/shopbind/getchild',
                    data:{'dept_id':$('#com-name-inp').attr('parent-id')},
                    dataType:'json',
                    async:true,
                    success:function(res){
        ...
        $('#position').click(function(){
            ...
            typeOfInputVal = 'position';
                ...
                $.ajax({
                    type:'get',
                    url:location.origin+'/op/shopbind/getchild',
                    data:{'dept_id':$('#position-inp').attr('parent-id')},
                    dataType:'json',
                    async:true,
                    success:function(res){
        ...
        function getData(res) {
            ...
            for(var i=0;i<res.length;i++){
                    ...
                    html += '<li id="' + res[i].id + '" parent-id="' + res[i].parent_id + '" class="">' + res[i].title + '</li>'
        ...
        function thinkData(){
            ...
            $.ajax({
                ...
                success:function(res){
                    ...
                    var html = '';
                    for(var i=0;i< res.length;i++ ){
                        if(pId == res[i].parent_id){
                            var thinkWord = $('#com-name-inp').val();
                            if(res[i].title.indexOf($.trim(thinkWord)) >= 0){
                                html += '<li id="' + res[i].id + '" parent-id="' + res[i].parent_id + '" class="">'+res[i].title+'</li>'
                            }
                        }
                    }
                    ...
                    for(var i=0;i< res.length;i++ ){
                        if(pId == res[i].parent_id){
                            var thinkWord = $('#com-name-inp').val();
                            if(res[i].title === $.trim(thinkWord)){
                                $('#position-inp').attr('parent-id', res[i].id);
                                break;
                            }else{
