 ...
 * Calendar.getInstance().parse('20160120102234') 需要保证14个数字（任意形式都可以，比如2016年04月07日 12点22分14秒），不够可以补0
 * @method Calendar
 * @author yanglang
 * @date 20160120
 */
define('utils/calendar.js',function(require,exports,module) {
    ...
    Calendar.prototype = {
        ...
        parse: function (timeObj) {
            ...
            if(type === '[object Date]'){
                ...
                timeObj = timeObj.replace(/[^\d]*/gm, '').replace(/^(\d{4})(\d{2})(\d{2})(\d{2})?(\d{2})?(\d{2})?$/, '$1/$2/$3 $4:$5:$6');
    ...
    /**
     * 获取Calendar实例
     * @param date optional 可选参数 可以传入一个日期对象或日期字符串或Calendar对象
