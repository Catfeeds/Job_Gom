...
(function (global, factory) {
   ...
   * date: 2016-06-28
   */
  Vue.filter("encodeHtml",function(str) { // 防止HTML、Javascript注入攻击
    return str ? str.replace(/[&<">'](?:(amp|lt|ldquo|rdquo|quot|gt|#39|nbsp|#\d+);)?/g, function(a, b) {
        if (b) {
            return a;
        } else {
            return {
  ...
  Vue.filter("imgbed", function(url, postfix) { //图床/缩略图
      if (!url) {
          return "";
      }
      var reg1 = /[^\/:]\/.*\//gi,
          res1 = url.match(reg1);
      if (res1 && res1.length) { //去除图片目录,缩略图路径只包含域名+图片名
          res1 = res1[0].substr(2);
          url = url.replace(res1, "");
      }
      // var reg2 = /_[0-9]+/gi; //去除之前的尺寸后缀 --线上不会有原图的 不用考虑之前的缩略图 20160720 wangchunpeng
      // url = url.replace(reg2, "");

      var reg = /\.(jpg|png|gif)/gi,
          res = url.match(reg);
      if (!res || !res.length) { //替换名字,添加后缀
          return url;
      }
      return url.replace(reg, '.' + postfix + res[0]);
  });
  /**
   * [description]  价格转换为元(默认是分),固定两位小数
   * @author wangchunpeng 2016-09-18T15:08:46+0800
   * @param  {[type]} price 价格 Number 分
   * @return {[type]} 价格 元
   */
  Vue.filter("price_fix2", function(price) {
