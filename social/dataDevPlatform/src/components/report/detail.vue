	...
	<div>
		...
		<div v-if="!tabVal">
			...
			<div class="form-actions">
					<button type="button" class="btn btn-sm btn-primary" @click="sendReport()">发送报表</button>
			...
			<modal :show.sync="showEmail" effect="zoom" title="发送邮件" :backdrop="true" ok-text="发送" cancel-text="取消" :callback="sendEmail">
				<div slot="modal-body" class="modal-body">
					<form>
						<div class="col-sm-12">
							<label class="col-sm-2 align-rt">收件人：</label>
							<div class="col-sm-10">
								<input class="form-control" v-model="form.receivers">
							</div>
						</div>
						<div class="col-sm-12">
							<label class="col-sm-2 align-rt">抄送：</label>
							<div class="col-sm-10">
								<input class="form-control" v-model="form.cc">
							</div>
						</div>
						<div class="col-sm-12">
							<label class="col-sm-2 align-rt">主题：</label>
							<div class="col-sm-10">
		...
		methods: {
			...
			sendEmail() {
				let obj = Object.assign({
					...
					controlParamList: JSON.parse(JSON.stringify(this.contrlArr))
				}, this.form);
				html2canvas(document.querySelector('.preview'), {
					...
					data.append('json_data', JSON.stringify(obj));
					data.append('file', dataURL);
					this.$http.post('report/mail', data, {headers: {'Content-Type': 'multipart/form-data', isFront: true}}).then(res => {
						actions.alert(store, {
							show: true,
							msg: res.msg,
							type: 'success',
