  ...
  data: function () {
	...
	methods: {
				...
				exif.getData(img, () => {
					...
					switch (rotate) {
						...
						case -3:
							...
							ctx.drawImage(img, 0, -height, width, height, 0, -height, width, height)
							break
						case 2:
						case -2:
							canvas.width = width
							canvas.height = height
							ctx.rotate(rotate * 90  * Math.PI / 180)
							ctx.drawImage(img, -width, -height, width, height, -width, -height, width, height)
						break
						case 3:
						case -1:
							canvas.width = height
							canvas.height = width
							ctx.rotate(rotate * 90  * Math.PI / 180)
							ctx.drawImage(img, -width, 0, width, height, -width, 0, width, height)
							break
						default:
							canvas.width = width
							canvas.height = height
		...
		moveImg (e) {
			...
			var nowX,nowY,resX,resY;
			nowX = e.clientX ? e.clientX : e.touches[0].clientX
      nowY = e.clientY ? e.clientY : e.touches[0].clientY

			resX = ~~(nowX - this.moveX);
			resY = ~~(nowY - this.moveY);

			//debug
			if(resX - this.ax < -this.trueWidth * this.scale + 10) {
				resX = this.ax - this.trueWidth * this.scale + 10;
			}
			if(resX - this.ax > this.trueWidth * this.scale - 10) {
				resX = this.ax + this.trueWidth * this.scale - 10;
			}
			if(resY - this.ay < -this.trueHeight * this.scale + 10) {
				resY = this.ay - this.trueHeight * this.scale + 10;
			}
			if(resY - this.ay > this.trueHeight * this.scale - 10) {
				resY = this.ay + this.trueHeight * this.scale - 10;
			}
      ...
      var nowY = e.clientY ? e.clientY : e.touches ? e.touches[0].clientY : 0
		...
		cropMove (e) {
			...
			window.addEventListener('touchstart', this.startMove)
...
<style scoped>
	.vue-cropper {
  	...
  	/*background-image: url('data:image/png;base64,iVBORw0KGgoAAAANSUhEUgAAABAAAAAQAQMAAAAlPW0iAAAAA3NCSVQICAjb4U/gAAAABlBMVEXMzMz////TjRV2AAAACXBIWXMAAArrAAAK6wGCiw1aAAAAHHRFWHRTb2Z0d2FyZQBBZG9iZSBGaXJld29ya3MgQ1M26LyyjAAAABFJREFUCJlj+M/AgBVhF/0PAH6/D/HkDxOGAAAAAElFTkSuQmCC');*/
		background-image: url("http://js.dev.meixincdn.com:1314/CDN8176/dist/20171212/images/adjustBg.png");
