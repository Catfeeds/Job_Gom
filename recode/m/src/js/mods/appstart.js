...
const AppstartClick = document.querySelector('#appstart-click')
const cookie = {
	parse() {
		try {
...
if (AppstartClick) {
    AppstartClick.addEventListener('click', _ => {/*
        document.querySelector('.m-downapp').style.display = 'none'
        document.querySelector('body').classList.remove('hasAppStart')*/
        const Days = 2
        const time = Days * 24 * 60 * 60 * 1000
        if(/^m\./.test(location.host)){
            cookie.set('mAdvertisingN','true','time',location.host);
        }else if(/^item\./.test(location.host)){
            cookie.set('itemAdvertisingN','true','time',location.host);
        }else if(/^prom\./.test(location.host)){
            cookie.set('promAdvertisingN','true','time',location.host);
        }else if(/^circle\./.test(location.host)){
            cookie.set('circleAdvertisingN','true','time',location.host);
        }
    })
}  
