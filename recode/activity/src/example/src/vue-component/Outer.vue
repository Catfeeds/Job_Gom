  ...
  <div class="outer" v-show="childOuter&&outer">
      ...
      <div class="bg">
        ...
        <div class="rule-cont">
          <div class="cont-text">
            ...
            <p>参与时间：2017.12.10 -2017.12.13</p>
            <p>优惠券使用时间：2017.12.10 -2017.12.13</p>
            <p> <strong>• 活动奖品兑换：</strong></p>
    ...
    <div class="info" v-show="info">
      <div class="bg">
        ...
        <div class="nickname">
          <input :class="{'pitch': nickname.check}" type="text" maxlength="10" name="tname" placeholder="输入昵称" @keyup="checkNickname" @change="checkNickname" ref="inputNickname">
        </div>
    ...
    <div class="adjust" v-show="adjust">
      <div class="bg">
        ...
        <div class="ocrop"   style="position: absolute;top: 50%;left: 50%;margin: -100px 0 0 -100px;">
          <croppa
    ...
    <div class="equip" v-show="equip">
      ...
      <div class="equip-btn clearfix" v-show="shareAppShow">
      ...
      <div class="android-btn btn" v-show="!shareAppShow"><a @touchend="ShareShow(true)" href="javascript:;"></a></div>
    </div>

    <div class="poster-hint" v-show="poster">
      <div class="hint-bg"></div>
      <div class="hint-text">长按保存图片，去朋友圈发图求温暖吧！</div>
    </div>
...
<script>
  ...
  export default {
    ...
    watch: {
      "nickname.value": function(val, oldVal) {
        if(val.length > 10)this.$refs.inputNickname.value = this.$refs.inputNickname.value.substring(0, 10);
      }
    },
    methods: {
      Toast(msg, time) {
      ...
      UAInof() {
        ...
        if( UA.indexOf('micromessenger') !== -1 ) {
          _vue.baseDataUA = 'wx'
        } else if( UA.indexOf('wechatmoments') !== -1 ) {
          _vue.baseDataUA = 'pyq'
       } else if( UA.indexOf('sina') !== -1 ) {
          _vue.baseDataUA = 'wb'
       } else if( UA.indexOf('qq') !== -1 ) {
          _vue.baseDataUA = 'qq'
       } else if( UA.indexOf('qzone') !== -1 ) {
          _vue.baseDataUA = 'qqkj'
      ...
      InfoShow(beal, once) {
        ...
        this.CroppaOption.refresh();
        // console.log(this.CroppaOption.refresh())
        document.querySelector('.croppa-container').querySelector('input').value = '';
        ...
        if(once) {
          ...
          if(/^[\u4E00-\u9FA5aa-zA-Z1-9]{0,10}$/ig.test(value)){
            if(value.length){
              if(value.length<=10) {
      ...
      EquipShow(beal) {
        let _vue = this;
        let android_app = $g.system.android ? true : false;
        var sp = $GLOBAL_CONFIG["link_login"].indexOf("?") == -1 ? "?" : "&";
        // var UA = window.navigator.userAgent.toLowerCase();
        // alert($g.env.app)
        // alert(android_app);
        var UA = window.navigator.userAgent.toLowerCase();
        ...
        if( UA.indexOf('micromessenger') !== -1 ) {
          ...
          wx.onMenuShareAppMessage({
              title: sina_title,
              desc: sina_desc,
              link: link_login + sp + 'cmpid=fx_wap_wx_yx_' + $GLOBAL_CONFIG['user_id'],
              imgUrl: sina_img ,
          ...
          wx.onMenuShareTimeline({
              title: sina_title,
              link: link_login + sp + 'cmpid=fx_wap_pyq_yx_' + $GLOBAL_CONFIG['user_id'],
              imgUrl: sina_img ,
              trigger: function (res) {
                  // 不要尝试在trigger中使用ajax异步请求修改本次分享的内容，因为客户端分享操作是一个同步操作，这时候使用ajax的回包会还没有返回
                  // alert('用户点击分享到朋友圈');
          ...
          wx.onMenuShareQQ({
              title: sina_title,
              desc: sina_desc,
              link: link_login + sp + 'cmpid=fx_wap_qq_yx_' + $GLOBAL_CONFIG['user_id'],
              imgUrl: sina_img ,
              success: function () {
                  // 用户确认分享后执行的回调函数
              },
          ...
          wx.onMenuShareQZone({
              title: sina_title,
              desc: sina_desc,
              link: link_login + sp + 'cmpid=fx_wap_qqkj_yx_' + $GLOBAL_CONFIG['user_id'],
              imgUrl: sina_img ,
              success: function () {
                  // 用户确认分享后执行的回调函数
              },
        ...
        if($g.env.app && android_app) {
          _vue.shareAppShow = false
        }
        if(this.tool.check) {
...
<<<<<<< HEAD
          var link_logins = link_login + '&cmpid=fx_wap_'+ _vue.baseDataUA +'_yx_'+ $GLOBAL_CONFIG['user_id'];
          alert(link_logins)
=======
          var sp = $GLOBAL_CONFIG["link_login"].indexOf("?") == -1 ? "?" : "&";
          var gomeShareUrl = $GLOBAL_CONFIG["link_login"] + sp + 'cmpid=fx_wap_'+ _vue.baseDataUA +'_yx_'+ $GLOBAL_CONFIG['user_id'];
>>>>>>> 7e98fbdf2ca2fba5154df9c6e2e2373f3de0b26c
          var param = {
            title: qq_zone_title,
            shareDesc: qq_zone_desc,
            shareImageUrl: qq_zone_img,
<<<<<<< HEAD
            shareUrl: link_logins,
=======
            shareUrl: gomeShareUrl,
>>>>>>> 7e98fbdf2ca2fba5154df9c6e2e2373f3de0b26c
      ...
      setToast(messages) {
          ...
          clearTimeout(_vue.timer);
          console.log(this.toastOnOff)
          if(dom){
            if(this.toastOnOff){
              dom.style.display = 'block';
              dom.innerHTML = '<div style="margin:0 auto; max-width: 50%; padding: 20px; border: 1px solid rgb(0, 0, 0); background: rgba(0, 0, 0, 0.8); text-align: center; color: rgb(255, 255, 255); font-size: 12px;">'+ messages +'</div>';
              this.toastOnOff = false;
            }
          }else{
            $("body").append('<div id="browser-share" style="position: fixed; z-index: 999; bottom: 20%; width: 100%; display:-webkit-box;display:-webkit-flex;display: flex; -webkit-box-pack: center; justify-content: center;"><div style="margin: 0 auto; max-width: 55%; padding: 20px; border: 1px solid rgb(0, 0, 0); background: rgba(0, 0, 0, 0.8); text-align: center; color: rgb(255, 255, 255); font-size: 12px;">'+ messages +'</div></div>');
          }

         _vue.timer = setTimeout(function(){
           $("#browser-share").hide();
           _vue.toastOnOff = true;
         }, 2 * 1000);
      },
      adjustFile(e, beal) {
        let _vue = this
...
>>>>>>> 7e98fbdf2ca2fba5154df9c6e2e2373f3de0b26c
      ...
      createBanner(beal) {
        var data={
        	...
        	"resImage":[],
        	"toolImage":[],
        	"codeImage":[$GLOBAL_CONFIG['activity_domain'] + 'qrcode/urlcode?url=' + encodeURIComponent($GLOBAL_CONFIG['link_login'])],
          "wSize": 0,
          "hSize": 0,
          "Xposition": 0,
          "Yposition": 0
        		...
        		function faceDraw(num){
                  ...
                  img.onerror= function() {
                    // alert('error')
          				...
          				img.src = 'http://activity.m.gome.com.cn/api/get_content?url=' + _vue.faceUrl;
        				}
        			} else {
        				bgDraw(0);
        			}
