...
class ChannelController extends BaseController {
    ...
    public function api_ids() {
        //topic/group/product
        $type = I( 'param.type', '' );
        //5912f9a1d15f8c64329f57ec,591451edd15f8c210d25db4f,58d8b2aed15f8c185d7ce573
        $ids = I( 'param.ids', '' );
 /*       if(strpos($ids,'|')) {
            ...
            foreach ($products_ids as $pid) {
                if (strpos($pid, '|')) {
                    $_pid = explode('|', $pid);
                    $productArr[$_pid[0]] = $_pid[1];
                }
            }
            $ids = implode(',', array_keys($productArr));
        }*/
...
//                    $new_data[ $v['id'] ]['price'] = convert_price( $v['price'] );
