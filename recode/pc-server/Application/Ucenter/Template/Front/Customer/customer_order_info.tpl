      ...
      <div class="bg-white">
        <div class="order-detail-box">
          ...
          <ul class="order-detail clearfix">
            ...
            <li class="clearfix">
              <label>退款编号：</label>
              <p data-node="goods_id"><{$senddata.order_info.money_id}></p>
            </li>
            <li class="clearfix">
              <label>退款金额：</label>
			...
			<if condition="$orderType eq 6 or $orderType eq 7" >
				<?php if (($linegoods['collectingMethod'] == 'CUSTOMER_SEND_BACK') && ($status == -103)) {?>
					<?php if (!empty($linegoods['mailAddress'])) { ?>
					<li class="no-float">
						<label>寄回地址：</label>
					<p><{$linegoods.mailAddress}></p>
					</li>
					<?php }?>
				<?php }?>

				<?php if (($linegoods['collectingMethod'] == 'GOME_PICKUP') && ($status == -103)) {?>
					<li class="no-float message">
						<label>退换货提示：</label>
						<p>您的商品需要商家上门取货。取货前会短信通知您取货时间。有问题请联系国美小美。<em>商家取货后，请点击“<strong>确认已取货</strong>”按钮，以免 耽误您的退款。</em></p>
					</li>
				<?php }?>
		  </if>
          </ul>
				<if condition=" $linegoods['collectingMethod']  eq 'CUSTOMER_SEND_BACK' "><!--买家寄回-->
				<?php if ($status == -103) { ?>
					<div class="tc"><a href="javascript:;" class="w120-h40 order-btn-orange sendgoods" data-node="clickBtn">发货</a></div>
				<?php }?>
				</if>

				<if condition=" $linegoods['collectingMethod'] eq 'GOME_PICKUP' "><!--上门取货-->
					<?php if ($status == -103) {?>
					<div class="tc"><a href="javascript:;" class="w120-h40 order-btn-orange getgoods" data-node="clickBtn">确认已取货</a></div>
					<?php }?>
				</if>
        </div>
      </div>
          ...
          <td colspan="3">
            <div class="products-by">
			  ...
			  <a href="<{$vo['good_id']|productDetailUrlGen=$vo['shopid'],###}>" target="_blank">
                  <div class="product-bx"><img src="<{$vo.good_image|getResizeImg=###,80,80}>" onerror="imgError(this)"></div>
