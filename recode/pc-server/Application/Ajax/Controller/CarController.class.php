...
class CarController extends BaseController
{
    ...
    public function put() {
        ...
        $sourceCodeArr = json_decode($source_code, true);
        $sid = '';
        $sid = C('sid')[$sourceCodeArr['sourceCode']];
        //有渠道时，渠道优先
        if(session('channel')){
        ...
        if($sid)
        {
            $sourceCodeArr['sid'] = $sid;
            $source_code = json_encode($sourceCodeArr);
            $this->car_v2->publicParamv2['sid'] = $sid;
        }
