...
//	public $regist_user_by_referral_code = 'user/get_user_by_referral_code.json'; // 检验推荐码
	...
	//第三方账号授权绑定
	public $gomeThirdPartyInfo = 'user/gomeThirdPartyInfo';

	//社交会员登录状态兑换
	public $login_token = 'user/loginTokenGenerateAction';
    ...
    public function saveCookie($data = array()) {
        ...
        if($data) {
            cookie('userId', authcode($data['loginId'], 'ENCODE', C('ENCRYPT_COOKIE_KEY')));
            cookie('nickName', authcode($data['loginName'], 'ENCODE', C('ENCRYPT_COOKIE_KEY')));
//            cookie('userInfo', authcode(json_encode($data), 'ENCODE', C('ENCRYPT_COOKIE_KEY')));
//            session("token_".$data['user']['id'], $data['loginToken']);  已经在SSO中注册
            //2017-1-10 登录后保存是否已经升级一账通的标识
//            session("authorize_".$data['user']['id'] ,$data['isAllowAuthorize']);
        }
    }
