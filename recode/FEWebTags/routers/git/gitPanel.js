...
module.exports = function(Router) {
	...
	Router.get('/gitPanel', function(req, res,next) {
		...
		request.get(constants.GITLABAPI + "/projects?access_token=" +
			...
			function(err,httpResponse,body){
				...
				console.log(constants.GITLABAPI + "/projects?access_token=" +
					req.session.userInfo.access_token + "&per_page=100");
				...
				if (json.message =='401 Unauthorized') {
					...
					for (i in json) {
						...
						if(json[i].permissions.project_access !== null && name_with_namespace === 'pluspcfed'){
							arr.push(json[i]);
						}
					}
					res.render('gitPanel', {
