...
module.exports = function(Router) {
  ...
  Router.post('/upgit', function(req, res) {
    ...
    var p = req.body.p;
    async(function() {
      var ret = await(exec("cd " + webhome + p + '/m/' + project +" && git pull origin develop",function(err,ret){
        var data = {};
        if(err){
          data = {
            type:'err',
      ...
      request.get(constants.GITLABAPI + "/projects/"+projectId+
          ...
          function(err,httpResponse,body) {
            ...
            if (json.message == "404 Branch Not Found") {
              ...
              if (fs.existsSync(webhome + p + '/m/' + projectName)) {
                res.send("already");
                var pr = {
                  type:'ret',
                  msg:'already'
                ...
                // exec('cd ' + webhome + p + '/m' + ' && git clone -b develop '+ projectUrl,function(err,ret){
                  exec('git clone -b develop ' + projectUrl + ' ' + webhome + p + '/m/' + projectName ,function(err,ret){
                      var data = {};
                      if(err){
  ...
  Router.get('/gitversion', function(req, res) {
    ...
    async(function(){
      if(fs.existsSync(webhome+ p +'/m/' + project + '/.git')){
        res.send('git');
        return false;
	    }
      if(fs.existsSync(webhome + p + '/m/' + project)){
        var ret = await (req.exec(webhome + p + '/m/' + project + ' && svn info', {
          precmd: 'cd'
        }));
  ...
  Router.get('/checkoutGit',function(req,res){
    ...
    try{
      if(fs.existsSync(webhome + p + '/m/' + project)){
        exec('mv ' + webhome + p + '/m/' + project + ' ' + webhome + p + '/m/SVN' + project + ' && cd ' + webhome + p + '/m' + ' && git clone -b develop ' + projectUrl, function (error, stdout, stderr) {
          if (error) {
            status = false;
            res.send(error);
            var pr = {
