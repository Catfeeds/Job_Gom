    ...
    Router.post('/gitAddServer', function(req, res) {
        ...
        portfinder.getPort(function(err, port) {
            ...
            async(function() {
                ...
                var gitProjectPathAddPn = git.replace('http://', 'http://' + req.session.userInfo.username.trim() + ':' + req.session.userInfo.password.trim() + '@');
                exec('git clone --depth=1 -b ' + branchName + ' ' + gitProjectPathAddPn + ' ' + gitProjectPath + projectPrefix + port,function(err, stdout, stderr){
        ...
        async(function() {
            ...
            for(var item in gitProjectJson){
                ...
                if(content['git'] && !fs.existsSync(content['diskPath'])){
                    cmd = 'git clone --depth=1 -b ' + content['branchName'] + ' ' + content['git'] + ' ' + content['diskPath'];
                }
                console.log(cmd);
                cmds.push(cmd);
            }
            ...
            try {
                ...
                if(gitProjectJson[serverPort]['git'] && fs.existsSync(gitProjectJson[serverPort]['diskPath'])){
                    cmd = 'rm -rf ' + gitProjectJson[serverPort]['diskPath']+
                        ' && git clone --depth=1 -b ' + gitProjectJson[serverPort]['branchName'] + ' ' + gitProjectJson[serverPort]['git'] + ' ' + gitProjectJson[serverPort]['diskPath'];
			console.log(cmd);
    ...
    function doUpdate(req, res) {
        ...
        async(function() {
          var cmds = servers.map(function(server) {
            ...
            //return 'rm -rf ' + gitProjectJson[server].diskPath + ' && git clone --depth=1 -b '+ gitProjectJson[server].branchName +' ' + gitProjectJson[server].git + ' ' + gitProjectJson[server].diskPath;
          });
