(function(){
	window.resourceNiche.forestallTopic = function(){
		var $forestallTopic = $('[data-small=pluspc-forestallTopic-boxS]')
		/*热门话题*/
		var $hotTopImg = $forestallTopic.find('[data-node=hotTop-img]');
		for(var i=0;i<$hotTopImg.length;i++){
			var tagW = $forestallTopic.find('.img-tag').eq(i).show().width()+1;
			$hotTopImg.eq(i).attr('data-width',tagW);
		}
		$forestallTopic.find('.img-tag').hide();		
		...
		var $listBtnBox = $forestallTopic.find('[data-node=list-btn]');
		var $listBtn = $listBtnBox.find('span');
		var $listUl = $forestallTopic.find('[data-node=interest-list]');
		var $listLi = $listUl.find('li');
		var $liLen = $listLi.length;
		var ulHtml = $listUl.html();
		var $interestWrap =$forestallTopic.find('[data-node=interest-wrap]');
