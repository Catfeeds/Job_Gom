...
const configFun = require('../common.conf.js');
...
HashMapPlugin.prototype.apply = function(compiler) {
    ...
    compiler.plugin('emit', (compilation, callback) => {
            ...
            if (extname === '.css' || extname === '.js') {
                ...
                if (key !== 'vendor') {
                    ...
                    if (vendorKey) {
                        ...
                        if (resultMap[key]['js'].indexOf(vendorPath) === -1) {
                            resultMap[key]['js'].unshift(vendorPath);
                        }
                        // console.log('vendorKey....', resultMap[key], key, localHash);
                        if (localHash.page[key]['js'].indexOf(vendorPath) === -1) {
                            localHash.page[key]['js'].unshift({
        ...
        this.addFileToAssets(compilation, `${path.relative(compiler.options.output.path, path.join(__dirname, '../'))}/flame/pgc/v-${process.env.NODE_ENV === 'production' ? 'pro' : process.env.NODE_ENV}.php`, `<?php\n\nreturn '${JSON.stringify(resultMap, null, 4)}';`)
