...
<div class="search-box" data-node="addTopBox">
  ...
  <em class="icon icon-search" data-action="addSearchBtn">&#xe9a5;</em>
</div>       
        ...
        <li data-pid="{{$value.PId}}">
          <img src="{{$value.img}}" alt="">
            <a href="javascript:;" data-action="delChanged">
              <em class="icon icon-dele-pic"></em>
            </a>
          </li>
        {{/each}}
    </ul>
  </div>
  <div class="num-pics">您可以添加 <span class="link-hover-red" data-node="searchChangeNum">{{maxlength}}</span><span class="deep-gray">/9 </span>个商品</div>
</div>