...
class CartController extends AuthController
{
    ...
    public function checkout()
    {
        ...
        $otherParam = json_decode($otherParam,ture);
        ...
        if($skuList && ($skuList = json_decode($skuList,true)))
        {
            ...
            {//收货地址为空时，js会在页面加入新加表单
                ...
                if(!empty($orderInfo['data']['orderPreviews']))
                {
                    ...
                    //重置订单数据容器
                    $orderList = array();
                    foreach($orderPriceSets as $shopId => $shopPrice)
                    {
                        if(isset($orderSets[$shopId]))
                        {//按照顺序重新追加
                            $orderList[] = $orderSets[$shopId];
                        }
                    }
                    //地址可配送时，取国美币信息
            ...
            $total['totalPaymentMoney'] = isset($orderInfo['data']['totalPaymentMoney']) ? $orderInfo['data']['totalPaymentMoney'] : 0;
