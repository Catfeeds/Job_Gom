...
<div class="wrap-box">
    ...
    <div class="shop-title clearfix">
		...
		<div class="search-warning" data-node='search-warning'>为您显示“<span><{$search_data['spellcheck']['suggestWord']}></span>”相关商品，仍然搜索“<a href="<{$search_data['spellcheck']['originWordUrl']}>"><span><{$search_data['spellcheck']['originWord']}></span></a>”<em class="icon" data-node='warning-icon'>&#xea5a;</em></div>
        </notempty>
		<div class="sort">
    ...
    <div class="shop-list">
        <ul class="clearfix">
            ...
            <a target="_blank" href="<{$vo.id|productDetailUrlGen=$vo['shopId'],###,$sourceCode}>" bp-data='{"event_id": "G000P006", "s_word": "<{$selectWords.keyword}>", "product_id": "<{$vo.id}>", "shop_id": "<{$vo['shopId']}>", "s_type": "product"}'>
            	...
            	<?php endif;?>
            	<img src="<{$vo.mainImage|getResizeImg=###,230,230}>">
                <div class="text">￥<span><{$vo.salePrice|convert_price=###}></span>
                    <p title="<{$vo.name|htmlspecialchars}>"><{$vo.name|htmlspecialchars}></p>
                </div>
            </a>
            </li>
            </volist>
