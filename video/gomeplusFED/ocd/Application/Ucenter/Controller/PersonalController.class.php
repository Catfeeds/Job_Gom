...
class PersonalController extends AuthController
{
	...
	public function getPersonalInfo()
	{
		...
		if($personalInfoRes)
		{
			...
			$personalInfoArr['birthday'] 	= $personalInfo['birthday'];
	...
	public function modPersonalInfo()
	{
		$defaultVal = '###';
		//备注：①性别需强制转换成整型
		$birthday 	= I('param.birthday', $defaultVal);
		$nickname 	= I('param.nickname', $defaultVal);
		$gender 	= I('param.gender', $defaultVal, 'intval');
		$facePicUrl	= urldecode(I('param.facePicUrl', $defaultVal));
		...
		{
			//URL检查是否是本站
			if (isTrustedDomain($param['$facePicUrl'])===true) {
