		...
		foreach($dataList['data']['collections'] as &$val)
		{
			...
			if(count($val['topic']['components']))
			{
				foreach($val['topic']['components'] as $valItem)
				{
					...
					{
                        $valItem['item']['originalPrice'] = convert_price($valItem['item']['originalPrice']);
                        $valItem['item']['price'] = convert_price($valItem['item']['price']);
                        $valItem['item']['salePrice'] = convert_price($valItem['item']['salePrice']);
                        $valItem['item']['skuHighestPrice'] = convert_price($valItem['item']['skuHighestPrice']);
                        $valItem['item']['mainImage'] = getResizeImg($valItem['item']['mainImage'], 230, 0);
                    }
					if($valItem['type'] == 'image')
					{
						$valItem['url'] = getResizeImg($valItem['url'], 230, 0);
					}
