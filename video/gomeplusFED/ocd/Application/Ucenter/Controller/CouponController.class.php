...
class CouponController extends  AuthController
{
	...
	public function  index(){
		...
		if($arrDisCountInfo['success'])
		{
			...
			foreach ($arrDisCountInfo['data']['coupons'] as $Item)
			{
				$arr = array();		
				$arr['money'] = $Item['money']/100;//金额
				$arr['minAmount'] = convert_price($Item['usageRule']['minAmount']);//满足使用金额
				$arr['shop_name'] = isset($Item['shop']['name']) ? $Item['shop']['name'] : "" ;
				$arr['shop_icon'] = isset($Item['shop']['icon']) ? $Item['shop']['icon'] : "" ;
				$arr['shop_id']   = isset($Item['shop']['id'])   ? $Item['shop']['id']   : "" ;
				$arr['start_time']= Date("Y.m.d",$Item['effectiveStartTime']/1000);
				$arr['end_time'] = Date("Y.m.d",$Item['effectiveEndTime']/1000);
				$intTime = time();
				$intEndTime = ($Item['effectiveEndTime']/1000) - 3*24*3600;
				$intRealEndTime = $Item['effectiveEndTime']/1000;
