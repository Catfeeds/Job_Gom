	          ...
	          <tr>
				...
				<volist name="order_item.orderItems" id="item_sku" key="sku_key" >
					...
					<a target="_blank" href="<{$item_sku.sku.item.id|productDetailUrlGen=$item_sku['mshop']['id'],###}>?skuid=<{$item_sku.sku.id}> "><img onerror="imgError(this)" src="<{$item_sku.sku.image|getResizeImg=###,80,80}>" class="order-img"></a>
					  ...
					  <a href="<{$i_domain}>order/showDataInfo?orderid=<{$order_item.id}>&skuid=<{$item_sku.sku.id}>&optype=1&goodid=<{$item_sku.sku.item.id}>" >退货</a>
					  </notempty>
					  <notempty name="item_sku.changeGoodsForFifteenDays">
					  <a href="<{$i_domain}>order/showDataInfo?orderid=<{$order_item.id}>&skuid=<{$item_sku.sku.id}>&optype=2&goodid=<{$item_sku.sku.item.id}>" >换货</a>
					  </notempty>
					  </div>
					</div>
				</volist>
	            ...
	            <td class="trading-status">
					...
					<a target="_blank" href="<{$i_domain}>order/orderLogistics?orderId=<{$order_item.id}>&statu=<{$order_item.status}>" >查看物流</a>
					</if>
					</notempty>
				</td>
	            <td class="order-operate">
					...
					<elseif condition="$order_item.status eq 2" />
						<a href="javascript:;" data-action="confirmReceipt" class="order-blank-btn marb14" data-id="<{$order_item.id}>" >确认收货</a>
						<if condition="$order_item.allowDelayConfirm eq 1">
						<a href="javascript:;" data-action="delayReceipt" class="order-blank-btn" data-id="<{$order_item.id}>">延迟收货</a>
						</if>
					<!--已收货-->
