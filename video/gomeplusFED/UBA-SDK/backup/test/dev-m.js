...
let mockApi = {
	fetch: 'http://localhost:8788/api/task',
	report: 'localhost:8788/api/task/report/'
}
let pageUrl = 'https://www-pre.gomeplus.com';
let url = `http://localhost:8788/api/page/html?pageUrl=${encodeURIComponent(pageUrl)}`;

test('api', async x => {

	let option = {
		url,
		filepath: path.join(__dirname, '../script/dev_manual.js'),
		close: false
	}
