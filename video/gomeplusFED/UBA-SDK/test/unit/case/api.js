...
test('api', async x => {
	...
	let option = {
		url,
		filepath: path.join(__dirname, '../script/api.spec.js'),
		close: false
	}
	...
	x.is(fetch_msg.event_msg, `api test operation: fetch task received. hosturl: ${url}`);
	x.is(report_msg.event_msg, `api test operation: report received. hosturl: ${url}`);
