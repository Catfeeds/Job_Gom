...
var jsonToQuery = function (a) {
    var add = function (s, k, v) {
        v = typeof v === 'function' ? v() : v === null ? '' : v === undefined ? '' : v;
        s[s.length] = encodeURIComponent(k) + '=' + encodeURIComponent(v);
    };
    var buildParams = function (prefix, obj, s) {
        var i;
        var len;
        var key;
...
var bindEvent = function (element, event, cb, capture) {
...
XAjax.prototype.handleResponse = function (xhr) {
...
XAjax.prototype.send = function (_a) {
    var _this = this;
    var _b = _a === void 0 ? {} : _a, _c = _b.method, method = _c === void 0 ? this.method : _c, _d = _b.url, url = _d === void 0 ? this.url : _d, _e = _b.options, options = _e === void 0 ? this.options : _e, _f = _b.headers, headers = _f === void 0 ? this.headers : _f;
    // Get XHR object
    var xhr = getXHR();
...
(_a = state.mockList).push.apply(_a, Array.prototype.map.call(datalist, function (x) { return x.innerText; }));
...
(_a$1 = state.urlList).push.apply(_a$1, Array.prototype.map.call(urls, function (x) { return ({ text: x.innerText.trim(), node: x }); }));
...
bindEvent(addBtn, 'click', function (event) {
    ...
    if (state.urlList.find(function (x) { return x.text === url; })) {
        alert('列表中已存在');
    }
    else {
...
function buildUrl(url) {
    var fwdurl = "http://" + host + "/api/page/html?url=" + encodeURIComponent(url) + "&id=" + state.socketid;
    return fwdurl;
}
var _a$1;
...
socket.on('log', function (data) {
    var UBA = data.UBA, UBAid = data.UBAid, spend = data.spend;
    var text;
...
socket.on('page', function (data) {
    var url = data.url, mobile = data.mobile;
...
socket.on('api', function (data) {
    var span = data.span, json = data.json, url = data.url;
...
function saveJsonData(_a) {
    var _b = _a === void 0 ? { callback: null, close: true } : _a, callback = _b.callback, close = _b.close;
    ...
    if (newname !== (oldname = state.modal.title)) {
        if (oldname) {
            ...
            if (state.mockList.includes(newname)) {
                ...
                state.modal.title = newname;
                saveJsonData({ close: false, callback: function () {
