(function(window, document, undefined) {
	...
	function getField(fieldName) {
		...
		if (!field) {
			for (var i = 0; i < customFieldCache.length; i++) {
				var fieldSet = customFieldCache[i],
					matches = fieldSet[0].exec(fieldName);
	...
	defineField("forceSSL", void 0, void 0,
		function (){
	...
	defineCustomField("\\&(.*)", function(matches) {
		var field = new Field(matches[0], matches[1]),
			fieldName = getFieldNameByProtocolParam(matches[0].substring(1));
	...
	*/
	function updateCookie(storage) {
		if ("cookie" === getStringField(storage, F_storage)) {
			var res = [],
				cookieName = storage.get(F_cookieName),
				cookiePath = processCookiePath(storage.get(F_cookiePath)),
				cookieDomain = processCookieDomain(storage.get(F_cookieDomain)),
				cookieExpires = storage.get(F_cookieExpires),
				clientId = getStringField(storage, F_clientId),
				subs = getDomain().split(".");
	...
	*/
	function buildClientId() {
		var navStr = navigator.appName + navigator.version + navigator.platform + navigator.userAgent + (document.cookie || "") + (document.referrer || ""),
			len = navStr.length;
	...
	*/
	function setClientId(storage, clientId) {
		var cookieName = storage.get(F_cookieName),
	...
	*/
	...
	controller.append = function() {
		var that = this,
			envs = that.process.apply(that, arguments);
