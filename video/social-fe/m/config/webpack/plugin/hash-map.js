...
HashMapPlugin.prototype.apply = function(compiler) {
	...
	compiler.plugin('emit', (compilation, callback) => {
		...
		this.addFileToAssets(compilation, `${path.relative(compiler.options.output.path, path.join(__dirname, '../'))}/flame-m/${configProject.project}/v-${process.env.NODE_ENV === 'production' ? 'pro' : process.env.NODE_ENV}.php`, `<?php\n\nreturn '${JSON.stringify(resultMap, null, 4)}';`)
