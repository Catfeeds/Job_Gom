<template>
  <ul @touchstart=touchstart($event) @touchmove=touchmove($event) @touchend=touchend($event)>
    ...
    <li v-if="configs.type == 'sxjg'" @click="goToxmxq(item)" v-for="item in configs.list" :class="setZxlbClassName(item)">
      <em class="line"></em>
      <a><span><u v-if="parseInt(item.isZx)">专项</u><i v-html="regText(item.text)"></i></span></a>
...
export default {
  ...
  methods: {
    regText(text){
      let reg = new RegExp(store.state.curProject.searchText, 'gi');
      if(store.state.curProject.reffer == 'search'){
        text = text.replace(reg, function (s){
          return '<small>'+ s +'</small>';
        });
      }else{
        // console.log('不是 search');
      }
      return text;
    },
