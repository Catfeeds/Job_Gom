    ...
    <div class="detailMain">
      ...
      <div class="detailcontainer">
        <ol>
          ...
          <li v-if="info.departs">
            <span class="detailItems">中心/部门：</span>
            <span class="detailexplainItems">{{info.departs}}</span>
          </li>
          ...
          <li class="zx_speed">
            ...
            <div class="detailexplainItems">
              <progress :value="parseInt(info.ext_speed_progress)" class="detailprogressItems" max="100"></progress>
              <span class="detailprogressCount">{{parseInt(info.ext_speed_progress)}}%</span>
            </div>
          </li>
          <li v-if="!!bearer.length">
            <span class="detailItems">承担人：</span>
            <span class="detailexplainItems">{{bearer.join('、')}}</span>
          </li>
...
<!--           <li v-if="!!members.length">
  ...
  <span class="detailexplainItems">
    <router-link to="/xmsx/xmxq/xmcy/2" tag="b" class="aaa">查看全部负责人</router-link>
          ...
          <li v-if="!!executor.length">
            <span class="detailItems">执行人：</span>
            <span class="detailexplainItems">{{executor.join('、')}}</span>
          </li>
          ...
          <li v-if="info.ext_comment_source">
            <span class="detailItems">批示来源：</span>
            <span :class="{detailexplainItems: true, margin: setClassName(3, info.ext_comment_source)}">{{info.ext_comment_source}}</span>
            <span class="switchBtn" v-if="setClassName(2, info.ext_comment_source)" @click="switchText($event, 3)"><!-- <em class="header-backicon iconn-1"></em> --></span>
          </li>
...
export default {
  ...
  computed:{
    ...mapState({
      ...
      bearer:state=>state.zxxqModule.bearer,
      executor:state=>state.zxxqModule.executor,
