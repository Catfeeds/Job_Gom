<template>
	...
	<fieldset>
		...
		<div class="form-group">
			...
			<div class="group-col clearfix" v-show="['pie', 'line', 'bar', 'map', 'scatter', 'radar', 'form', 'formContent'].includes(info.type)">
				<div class="group-col group-col-float">
					...
					<div class="inB" v-if="['line', 'bar', 'map', 'scatter'].includes(info.type)">
						<span>X轴坐标单位：</span>
						<dropdown>
					...
					<div class="inB" v-if="info.type === 'pie'">
						<span>选择数据条件：</span>
						<dropdown>
					...
					<!-- legend start -->
					<div class="inB" v-if="info.type !== 'form' && info.type !== 'formContent'">
						<span>legend(图例)：</span>
						<dropdown>
	...
	export default {
		...
		methods: {
			...
			getComponentData(item, e, child, index, paginationConfig) {
				...
				let isChart = ['line', 'bar', 'pie', 'map', 'scatter', 'radar'].includes(type);
				...
				this.$http({
					...
					if (res.data && res.data.result && res.data.result !== 'null') {
						...
						if (['line', 'bar', 'pie', 'map', 'scatter', 'radar'].includes(type)) {
