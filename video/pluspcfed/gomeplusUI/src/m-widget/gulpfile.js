...
//  .pipe(imagemin({ optimizationLevel: 0, progressive: true, interlaced: true }))
    .pipe(gulp.dest('../../../ui-html/m-w/images'))
...
gulp.task('pug', function() {
    ...
    .pipe(gulp.dest('../../../ui-html/m-w/html'))
});
...
//gulp.watch('src/sass/module/*.scss', ['styles2']);
 ...
 // Watch image files
  ...
  gulp.watch(['../../../ui-html/m-w/css/**','../../../ui-html/m-w/html/**','../../../ui-html/m-w/images']).on('del', livereload.changed);
...
gulp.task('sass', function() {
  return sass('sass/**/*.scss',{ sourcemap: true })
    .on('error', sass.logError)
	.pipe(gulp.dest('../../../gomeplusUI-demo/m-widget/css'))
    .pipe(notify({ message: '编译已完成' }));
});
...
gulp.task('img', function() {
  return gulp.src('images/**/*')
//  .pipe(imagemin({ optimizationLevel: 0, progressive: true, interlaced: true }))
    .pipe(gulp.dest('../../../gomeplusUI-demo/m-widget/images'))
    .pipe(notify({ message: '老大，您的图片压缩旨意已完成' }));
});
...
gulp.task('jade', function() {
    ...
    .pipe(gulp.dest('../../../gomeplusUI-demo/m-widget/html'))
});
...
gulp.task('del', function() {
    del([
		'../../../gomeplusUI-demo/m-widget/css',
		'../../../gomeplusUI-demo/m-widget/html',
		'../../../gomeplusUI-demo/m-widget/images'
	],{force:true})
});
...
//gulp.watch('src/sass/module/*.scss', ['styles2']);
 ...
 // Watch image files
  ...
  livereload.listen();
  // Watch any files in dist/, reload on change
  gulp.watch(['../../../gomeplusUI-demo/m-widget/css/**','../../../gomeplusUI-demo/m-widget/html/**','../../../gomeplusUI-demo/m-widget/images']).on('del', livereload.changed);
