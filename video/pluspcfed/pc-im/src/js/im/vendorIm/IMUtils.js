...
DepencyFunc.getServer = function(appId, imUserId, token, callBack, args, error) {
    ...
    $.ajax({
        ...
        success: function(data) {
            IMConstants.IM_Server = data.data.webSocketUrl;
...
function InitConf(imConstants, callback, token) {
    ...
    if (!IMConstants.IM_Server) {
        DepencyFunc.getServer(IMConstants.IM_APP_ID, IMConstants.IM_UID, token, [null, null]);
    }
...
function sendHeartBeat() {
    ...
    if (!isNaN(Global.lastTime) && Global.lastTime != 0) {
        ...
        if (Global.lastLoginTime && (!Global.loginStatus || Global.loginStatus == false)) {
            webSocketRebuild();
        }
    } else {
...
function sendLogin() {
    ...
    if (IMConstants.IM_TOKEN && IMConstants.IM_Server) {
        ...
        if (Global.lastLoginTime && now - Global.lastLoginTime < 0.3 * IMConstants.IM_HEART_BEAT_TIME) {} else {
...
function sendImMsg(obj) {
    ...
    var groupId = "";
    var msg = "";
    var mesgId = "" + new Date().getTime() + "!" + Math.random(10000);
    if (obj.option.userType == 1) {
        groupId = obj.option.imUserId + "_" + "9999999997" + "_" + obj.option.shopId; //商家客服
        msg = new ConsultImMsg().setCustomerId(9999999997).setImMsg(new ImMsg().setMsgId(mesgId).setMsgType(obj.option.msgType).setSenderId(obj.option.imUserId).setGroupId(groupId).setGroupType(obj.option.groupType).setMsgBody(obj.option.msgBody).setAttch(obj.option.attch).setExtra(obj.option.extra)).setShopId(obj.option.shopId);
    } else if (obj.option.userType == 2) {
        groupId = obj.option.imUserId + "_" + "9999999999"; //小美客服
        msg = new ConsultImMsg().setCustomerId(9999999999).setImMsg(new ImMsg().setMsgId(mesgId).setMsgType(obj.option.msgType).setSenderId(obj.option.imUserId).setGroupId(groupId).setGroupType(obj.option.groupType).setMsgBody(obj.option.msgBody).setAttch(obj.option.attch));
    }
    var headParam = new Object();
    headParam.receiveId = 0;
    var msgStr = MsgUtils.initJsonMsg(headParam, CommandConstants.CMD_CONSULT_IM_MSG, msg);
    var msgId = sendMsg(msgStr, mesgId);
    return msgId;
}
