...
var entry = {
    vendor: ['gome-error-report','module/ger','module/globalConfig', 'module/backTop', 'jquery', 'dialog', 'io/url', 'cookie', 'io/fetch', 'module/header', 'utils/buriedPoint', 'module/meidian-header']
};

if( env.dev || env.prd ) {
    entry.vendor.unshift('./bin/isDev');
}
var getEntry = function() {
    var pages = fs.readdirSync(pagePath);
    pages.forEach(function(dir, index, arr) {
        if (dir.indexOf('.') !== 0) { // .svn的隐藏目录
