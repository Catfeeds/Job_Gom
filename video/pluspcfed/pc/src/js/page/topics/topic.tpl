    ...
    <div class="mg-negative">
        ...
        {{/if}}
            <a href="{{topic.user.id | othersLink}}" target="_blank">
                <em class="icon-lysy">
                    <img src="{{topic.user.facePicUrl}}" title="{{topic.user.nickname}}" onerror="imgError(this,'h')">
                </em>           
            {{topic.user.nickname}}
            </a>
        </span>
        ...
        <div class="list-img">
            <a href="{{topics.groupDomain}}topic/{{topic.id}}.html" target="_blank" bp-data='{"event_id": "{{topics.event_id}}", "group_id": "{{topics.group_id}}", "topic_id": "{{topic.id}}"}'>
                <!-- 话题详情展示 -->
                <img src="{{topic.new_components.item.item.mainImage}}" title="" onerror="imgError(this);">
            </a>
        </div>
        <p class="list-price">
        ...
        <div class="list-img">
            <a href="{{topics.groupDomain}}topic/{{topic.id}}.html" target="_blank" bp-data='{"event_id": "{{topics.event_id}}", "group_id": "{{topics.group_id}}", "topic_id": "{{topic.id}}"}'><img src="{{topic.new_components.image.url}}" title="" onerror="imgError(this);"></a>
        </div>
        {{else if topic.new_components.video}}
         <div class="list-img">
            <a href="{{topics.groupDomain}}topic/{{topic.id}}.html" target="_blank" bp-data='{"event_id": "{{topics.event_id}}", "group_id": "{{topics.group_id}}", "topic_id": "{{topic.id}}"}'>
                <img src="{{topic.new_components.video.coverImage}}"  title="" onerror="imgError(this);" style="min-height:130px">
