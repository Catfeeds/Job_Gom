...
$(document).on('scroll', function() {
    if(!$goodBox.length) return;
    if(!loadFlag) return;
    if ($goodBox.height() + $goodBox.offset().top >= ($(window).height() + $(window).scrollTop())) return;
    loadFlag = false;
    var text = $moreBtn.html(),
        _url = '',
        _data = {};
    if ($_CONFIG.isSearch == '1') {
        _url = $_CONFIG.meidian_domain + url.get('searchGetMore');
        _data = {
            shop_id: ~~$_CONFIG.shopId,
            pageNum: morePage,
            word: $_CONFIG.word
    ...
    function getMore(){
        $moreBtn.addClass('disabled').html('<span><img src="' + $_CONFIG.imgpath + '/images/public/loading.gif">正在加载...</span>');
        fetch.get(_url, {
            data: _data
            ...
            /*if (length < 20) {
                ...
                $moreBtn.html('<span class="user-goods-nomore"><span>没有更多商品了</span></span>');
                return;
            } else {
                $moreBtn.removeClass('disabled').html(text);
                loadFlag = true;
                morePage++;
            }
            if ($_CONFIG.type == '2') {
            ...
            $moreBtn.html('<span class="user-goods-moreLogo" data-action="retry"><span class="user-goods-moreIcon">加载失败，点击重试</span></span>');
            //获取数据失败后点击可重新获取
            $('[data-action=retry]').on('click',getMore);
        });
    }
