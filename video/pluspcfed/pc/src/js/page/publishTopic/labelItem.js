	...
	LenovoBandShow(req) {
		...
		fetch.get(url.get('tagSearch'), {
            ...
            if (res && res.code === 200 && res.success) {
				...
				res.data.tags.every(function(item, index) {
					// console.log(item);
					...
					if(index == 0) {
						$(_this.elementNode.listSearch).append(`<li class="list-data label-active" data-node="list-frist" title="${item.name}" data-id="${item.id}"><span>${item.name}</span></li>`);
					} else {
						$(_this.elementNode.listSearch).append(`<li class="list-data" title="${item.name}" data-id="${item.id}"><span>${item.name}</span></li>`);
					}
					scroll&&scroll.refresh();
					return true;
