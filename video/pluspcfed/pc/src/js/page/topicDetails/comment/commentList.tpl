...
<dl class="{{dlShowBorder }} "  {{if userSubmit}} data-node="userSubmit"{{/if}} data-allNum="{{subReplyQuantity}}" data-commId="{{backTopicId}}">
		<dt>
		...
		<dd data-ddList="{{backTopicId}}">
				<div class="circle-comments-title clearfix"   data-topicType={{topicType}} data-htId={{backTopicId}}  data-tid={{topicId}}>
					<p class="comments-replay" data-node="comReplay">
						...
						{{/if}}
						{{if canDelete}}
							<a href="javascript:;" class="replay-report" data-delId="{{backTopicId}}" data-topicId="{{topicId}}" data-action="del-discuss" >删除</a>
							<span class="line"></span>
						{{/if}}
						<a href="javascript:;" data-node="secondCom_Key" data-comId="{{backTopicId}}"  data-replayUser={{username}} data-publish="1" data-allNum="{{subReplyQuantity}}">回复</a>
					</p>
					<a href="javascript:;" class="topic-parise {{if isLike}} actived {{/if}}" data-aciton="discuss-like" data-id="{{backTopicId}}" title="{{if isLike}}取消赞{{else}}赞{{/if}}">{{userQuantity}}<span></span></a>
							...
							<div class="reply-details merchant">
									...
									<div class="reply-contxt">
											...
											<div class="price clearfix">
													...
													{{if pricetype === 'RUSHBUYPRICE'}}
														<div class="buy">
															<em class="icon-rob"></em>
														</div>
													{{/if}}
													{{if pricetype === 'TUANPRICE'}}
														<div class="buy">
															<em class="icon-tuan"></em>
														</div>
													{{/if}}
											</div>
									</div>
				...
				<div class="comments-s clearfix"  data-topicType={{topicType}} data-htId={{backTopicId}}  data-tid={{topicId}} data-hide="0">
					...
					<div class="topic-second-comment">
						{{if value.sCanDelete}}
							<a href="javascript:;" class="hidden" data-topicId="{{topicId}}" data-delId="{{value.commentId}}" data-action="del-discuss" data-type="second">删除</a>
							<dfn class="hidden">&nbsp;|&nbsp;</dfn>
						{{/if}}
						<a href="javascript:;" data-getTopicId={{backTopicId}} data-node="secondCom_Key" data-replayUser={{value.replyUserName }} data-publish="0" data-comId="{{value.parentCommentId}}" data-replyCommentId ={{value.replyCommentId}} data-replyUserId={{value.replyUserId}} data-imId={{imId}}>回复</a>
					</div>
