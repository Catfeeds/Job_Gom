    ...
    if (comment_Msg || imgPics.length || $addImgGoods.hasClass("hasGoods")) {
        ...
        fetch.post(url.get('commentFirstV2'), {
            ...
            if (data.success) {
                ...
                var basics = {
                    "showSecComm": "hide",
                    "mall_domain": Msg.item && Msg.item.itemId && ($.trim(Msg.item.pricetype) === 'PUSHBUYPRICE' || $.trim(Msg.item.pricetype) === 'TUANPRICE') ? $_CONFIG["product_item_tuan_gome"] : mall_domain,
                    "itemsPic": itemsPic,
                    ...
                    "kid": Msg.kid,
                    "canDelete": true,
                    "pricetype": Msg.item ? $.trim(Msg.item.pricetype) : '',
                    "itemId": Msg.item && Msg.item.itemId ? Msg.item.itemId : '',
                    "userQuantity": 0
                ...
                var $warp = $("[data-circlecom='" + topicid + "']");
                $warp.find('dl').length === 0 ? $warp.append(item) : $warp.find('dl').eq(0).before(item);
