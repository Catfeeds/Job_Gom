...
class Log {
	 ...
	 * @param string $destination  写入目标
	 * @return void
	 * @author guojianing@gomeplus.com
	 */
	static function writeNew($message, $type='',$destination='') {
		if(!self::$storage){
			$type	=	$type ? : C('LOG_TYPE_NEW');
			$class  =   'Think\\Log\\Driver\\'. ucwords($type);
			$config['log_path'] = C('LOG_PATH_NEW');
			self::$storage = new $class($config);
		}
		if(empty($destination)){
			//$destination = C('LOG_PATH_NEW').date('Y').'/'.date('m').'/'.date('d').'.log';
            $destination = C('LOG_PATH_NEW').date('y_m_d').'.log';
		}
		self::$storage->write($message, $destination);
	}
