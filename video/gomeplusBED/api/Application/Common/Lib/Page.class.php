...
class Page{
	...
	public function show($num, $perpage, $curr_page, $mpurl = '',$join_str='')
	{
		...
		if($num > $perpage) {
			...
			if($curr_page != 1) {
				$t = $curr_page - 1;
				$multipage .= " <a href =\"" . $mpurl .$join_str ."&page=" . $t .  "\" class='pageup'  title='上一页' ><em></em>上一页</a> ";
			} else {
				$multipage .= ' <a class="pageup pageup-dis disabled" href="javascript:void(0);" title="上一页"><em></em>上一页</a> ';
			}
			...
			//第一页...
			if($from > 1) {
				$multipage .= " <a href =\"" . $mpurl .$join_str ."&page=1".  "\">1</a> ";
				$multipage .= ' <span>…</span>';
			}
			...
			for($i = $from; $i <= $to; $i++) {
				if($i != $curr_page) {
					$multipage .= " <a href=\"" . $mpurl . $join_str."&page=" . $i  . "\">" . $i . "</a> ";
				} else {
			...
			if($curr_page != $pages) {
				$t = $curr_page + 1;
				$multipage .= " <a href =\"" . $mpurl  .$join_str ."&page=" . $t.  "\" class='pagedown' title='下一页'>下一页<em></em></a> ";
			} else {
				$multipage .= ' <a class="pagedown pagedown-dis disabled" href="javascript:void(0)" title="下一页">下一页<em></em></a> ';
...
// 	    }
	    ...
	    if($num > $perpage) {
	        ...
	        if($curr_page != 1) {
	            ...
	            if($t == 1){
	                ...
	                if(isset($params[0]) && $params[0] == 0){
	                    ...
	                    $multipage .= " <a href =\"" . $join_str  ."_" . $t .  "$url_extension\" class='pageup'  title='上一页' ><em></em>上一页</a> ";
	                }
	            }else{
	               $multipage .= " <a href =\"" . $join_str  ."_" . $t .  "$url_extension\" class='pageup'  title='上一页' ><em></em>上一页</a> ";
	            }
	        ...
	        for($i = $from; $i <= $to; $i++) {
	            if($i != $curr_page) {
	                if($i == 1){
	                   ...
	                   $multipage .= " <a href=\"" . $join_str ."_" . $i  . "$url_extension\">" . $i . "</a> ";
	                }
                } else {
                    $multipage .= " <a class='active' href='javascript:void(0);'>" . $i . '</a> ';
	            }
	        ...
	        if($curr_page != $pages) {
	            $t = $curr_page + 1;
	            $multipage .= " <a href =\"" . $join_str  ."_" . $t.  "$url_extension\" class='pagedown' title='下一页'>下一页<em></em></a> ";
	        } else {
	            $multipage .= " <a class='pagedown pagedown-dis disabled' href='javascript:void(0)' title='下一页'>下一页<em></em></a> ";
	        }
	        ...
	        if(($pages>$this->page_show_row) && ( ($curr_page+$this->page_offset+1) < $pages) && ( $pages != $this->page_show_row + 1 )){
	            $multipage .= ($this->show_more) ? ' <span>…</span>' : '';
	            $multipage .= " <a  href=\"" . $join_str ."_" . $pages  . "$url_extension\">" . $last_label . "</a> ";
