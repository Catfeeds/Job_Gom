 ...
 * @return mixed|string
 */
function productDetailUrlGen($shopId, $productId, $skuId = 0)
{
    $url = '//'.C('GOME')['URL']['MALL_ITEM_URL'];
    if($productId){
        $url .= $productId;
    }
	if($skuId)
...
{
    if( $topicId === "" ) return ;
	$url = '//'.C('GROUP_URL').'topic/'.$topicId.'.html';
    return $url;
}
...
function groupDetailUrlGen($groupId,$isEssence=0)
{
    if( $groupId === "" ) return ;
    if($isEssence == 1){
	   $url = '//'.C('GROUP_URL').'circle/'.$groupId.'_1.html';
    }else{
	   $url = '//'.C('GROUP_URL').'circle/'.$groupId.'.html';
    }
...
function userInfoUrlGen($userId,$currentUserId=0,$loginStatus=0)
{
    ...
    if($userId == $currentUserId && $loginStatus==3){
        $url = '//'.C('UCENTER_URL').'index';
    }else{
    	$url = '//'.C('GROUP_URL').'ta/'.$userId.'.html';
    }
...
function getGFSResizeImg($orgin_url,$width,$hight,$action){
    ...
    if(in_array($img_key ,$imgSizeArr)){
        ...
        if($repString && ( in_array($repString,$imgSize) || in_array($repString, $thumbnailSize)) ) {
...
function getMeixinImg($orgin_url,$width,$hight){
    ...
    if(isset($imgSize[$img_key])){
        ...
        if(isset($img_arr[0]) && isset($img_arr[2]) && isset($img_url['filename'])){
            return $orgin_url.'.'.$imgSize[$img_key].'.'.$defaultExt;
        }
    }
    return $orgin_url;
}
...
function isTrustedDomain($referer){
    $domain = ['.gomeplus.com','.meixincdn.com', '.atguat.net.cn','.gomein.net.cn','.uatplus.com','.gome.com.cn'];
 ...
 *
 */
function num_format($n,$param=1){
    if( !isset($n) ) return ;

    if($n >= 10000) {
        return sprintf( "%01.".$param."f",$n/10000 ).'万';
    }
    return $n ;
}


function filterProtocol($orginUrl){
    if(!$orginUrl) return;
    $orginUrl = str_replace(['http:','https:'], '', $orginUrl);
    return $orginUrl ;
}