...
function A($name,$layer='',$level=0) {
    ...
    //by:maoxiaoqi  扩展增加统一empty
    if( !APP_DEBUG && $name == 'Empty' && !empty( C('EXT_DEFAULT_C_EMPTY') ) && strpos( $class,'\\' ) ) {
        $arr = explode( '\\', $class );
        $arr[0] = ucfirst( C('EXT_DEFAULT_C_EMPTY') );
        $class = implode( '\\', $arr );
    }
...
function session($name='',$value='') {
    ...
    if(is_array($name)) { // session初始化 在session_start 之前调用
        ...
        //设定session垃圾回收时间
        if(isset($name['maxlifetime'])){
            ini_set('session.gc_maxlifetime',  $name['maxlifetime']);
        }
