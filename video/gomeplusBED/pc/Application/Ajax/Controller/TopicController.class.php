...
class TopicController extends BaseController
{
    ...
    public function first_v2() {
        ...
        $param['skuId']    = I('param.skuid', '');//如果topicType = 1 那么这个字段必填
		...
		*/
        ...
        //清除缓存
        deleteSocialCache('all', $groupId, $param['topicId']);
     ...
     * 删除话题一级回复
     */
    public function first_v2_del(){
        //圈子id
        $group_id = I('post.group_id', '', 'strval');

        //话题id
        $topic_id = I('post.topic_id', '', 'strval');

        //被删除的一级回复id
        $del_id = xss_clean(I('post.del_id', '', 'strval'));

        if(empty($group_id) || empty($topic_id) || empty($del_id)){
            $this->outError(\Think\ErrorCode::PARMA_ERROR);
            exit;
        }

        $param['id'] = $del_id;
        $topic_v2_service = D('Ajax/TopicV2');
        $res = $topic_v2_service->deleteData($topic_v2_service->first_topic_del, $param);

        //清除缓存
        deleteSocialCache('all', $group_id, $topic_id);
     ...
     * 二级回复
     * */
    public function mshop_second_v2() {
    ...
    public function second_v2() {
        ...
        //圈子id
        $groupId = I('param.group_id', '');

        $param['topicId']    = I('param.topicid', '');//话题id
        $param['content']    = xss_clean(I('param.content', ''));//内容字数在200字内含200字
        $param['topicReplyId']    = I('param.topic_reply_id', '');//一级话题回复id
        $param['topicSubReplyId']    = I('param.topic_subreply_id', '');//被回复的二级回复ID
        if(empty($groupId) || empty($param['topicId']) || empty($param['topicReplyId'])){
            $this->outError(\Think\ErrorCode::PARMA_ERROR);
        }
        //内容不能为空
       /* if( !$param['content'] ){
        ...
        //清除缓存
        deleteSocialCache('all', $group_id, $param['topicId']);
    ...
    public function second_v2_del(){
        ...
        $param['id'] = $del_id;
        $topic_v2_service = D('Ajax/TopicV2');
        $res = $topic_v2_service->deleteData($topic_v2_service->second_topic_del, $param);
...
//                 print_r($arrItem);exit;
    ...
    public function del(){
        ...
        if(empty($groupId) || empty($param['id'])) {
            $this->outError(\Think\ErrorCode::PARMA_ERROR);
            exit;
        }
    ...
    */
    ...
    public function create(){
        ...
        if(isset($_POST['from']) && $_POST['from']==1 && isset($_POST['tid']) ){
            if( strlen($_POST['tid']) != 24){
    ...
    private function addTopic($data){
        ...
        $_P = $data;
        //美号文章上传图片为50
        $limit_nums = $_P['extTopicType'] == 8 ? 50 : 20;
        $good_nums = 9;

        //标签验证|每个话题必须携带一个标签&&美店不带标签

        if( $_P['extTopicType'] != 7 && empty($_P['labels']) ){
            $this->outError(\Think\ErrorCode::TAG_IS_EMPTY);
            exit;
        }

        //长文、短文| 0:长文,1:短文
        $_P['mshopTopicType'] = isset($_P['mshopTopicType']) ?  $_P['mshopTopicType']  : 0;
        ...
        if(isset($_P['extTopicType'])){
            $_P['extTopicType'] = intval($_P['extTopicType']);
        }
        unset($_P['from']);
        unset($_P['tid']);
        unset($_P['group']);
...
/*
    ...
    /**
     * 置顶|加精 - 取消置顶|取消加精
     * @param int type 加顶或者取消 1：加顶 0：取消
     * @param int ac ,0 :置顶 1:加精
     */
    public function actopic(){
        $actionArr  = array(0=>'isUpper',1=>'isEssence');
        $id         =  xss_clean(I('post.topicId','','strval'));
        $groupId    = xss_clean(I('post.groupId',0,'strval'));
        $action     = I('post.ac',0,'intval');
        $type       = (bool)I('post.type',0,'intval');
        ...
        $topic = D('Group/Topic');
        $topic->publicParamv2['id'] = $id ;
        $param[$actionArr[$action]] = $type ;
        $param['groupId'] = (string)$groupId ;
        $result = $topic->putData($topic->base_topic,$param);

        //清除缓存
        deleteSocialCache('all', $groupId, $id);
    ...
    public function relatedTopics(){
        $param = [];
        $param['boxType'] = I('param.boxType',1,"intval");
        $param['topicId'] = I('param.tid','','string');
        if( !$param['topicId'] ){
            $this->outError(\Think\ErrorCode::PARMA_ERROR);
            exit;
        }
        //print_r($param);die;
        $topic = D('Group/Topic');
        $result = $topic->getData($topic->relatedTopics,$param);
    ...
    /*
     * 批量获取接口 美店三期挪动到ajax的方法 之前在group模块的channel控制中 不能通用
     *
     * @return json
     * */
    public function api_ids() {
        ...
        switch( $type ) {
            ...
            case 'product':
                //商品
                $product = D( "Services/Product" );
                $data = $product->get_products_price( $ids );
