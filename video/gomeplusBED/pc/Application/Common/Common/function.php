 ...
 * @return mixed|string
 */
function productDetailUrlGen($shopId, $productId, $skuId = 0, $kid = '',$stid = 0 )
{
 ...
 * @return mixed|string
 */
function mshoptopicDetailUrlGen($topicId)
{
    if( $topicId === "" ) return ;
    $url = APP_HTTP.C('MEIDIAN_URL').'sayinfo/'.$topicId.'.html';
    return $url;
}
...
function isTrustedDomain($referer){
    $domain = ['.gomeplus.com','.meixincdn.com', '.atguat.net.cn', '.atguat.com.cn','.gomein.net.cn','.uatplus.com','.gome.com.cn','gome.cn'];
    $referer = parse_url($referer);
    $isTrue = false;
    foreach ($domain as $d){
        if( false !== stripos($referer['host'],$d)){
            $isTrue = true;
            break;
        }
    }
