...
class Rsyslog {
    ...
    private static function initialize($options)
    {
        ...
        //不需要记录图片的Base64编码内容
        if(isset($request_params['src']) && $request_params['src']){
            preg_match('/^(data:\s*image\/(\w+);base64,)/', urldecode($request_params['src']), $result);
            if(!empty($result)){
                $request_params['src'] = '...';
            }
        }
        ...
        self::$params['user_id']        = isset($_COOKIE['SSO_USER_ID']) ? $_COOKIE['SSO_USER_ID'] : ''; // 解密
        self::$params['scn']            = !empty($_COOKIE['SCN']) ? $_COOKIE['SCN'] : '';
