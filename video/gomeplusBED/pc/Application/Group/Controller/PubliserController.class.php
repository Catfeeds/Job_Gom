...
class PubliserController extends BaseController
{
    ...
    {
        $param = [];
        $group_id = I('param.group_id/s', '');
        $this->publiser->publicParamv2['id'] = $group_id;


        $old_name = trim(I('param.old_name/s', ''));
        $new_name = trim(I('param.new_name/s', ''));
        if($old_name != $new_name){
            $param['name'] = $new_name;
        }
        $param['category']['id'] = I('param.id/d', '');
        $param['introduction'] = I('param.introduction/s', '');
        $param['icon'] = I('param.icon/s', '');
        $param['approvalType'] = I('param.approvalType/d', 0);
        $param['maxUsers'] = I('param.maxUsers/d', 1000);

        //var_dump($param);die;

        if (empty($group_id) || $param['category']['id'] === ''){
            $this->outError(\Think\ErrorCode::PARMA_ERROR);
        }

        //过滤敏感词
        sensitive($param['name']);
        if(!empty($param['introduction'])){
            sensitive($param['introduction']);
        }

        $result = $this->publiser->putData($this->publiser->create_circle, $param);
