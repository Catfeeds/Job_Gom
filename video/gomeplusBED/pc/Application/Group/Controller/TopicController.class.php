...
class TopicController extends BaseController {
    ...
    public function index() {
        ...
        //print_r($g_m_infos);exit;

        $is_myself = false;
        if($g_m_infos['user']['id'] ==  $this->userId){
            $is_myself = true;
        }
        if(empty($g_m_infos)){
        ...
        $this->assign( 'gid', $gid);
        $this->assign( 'is_myself', $is_myself);  //该圈子是否是当前登录用户的圈子的标识
        ...
        //seo需要有圈子对应的wap地址
        $this->assign( 'mobile_url','//'. C('WAP_CIRCLE_URL').'circle-'.$gid.'.html');
    ...
    private function _cicleError($data){
        ...
        $this->assign( 'flag', $flag);  //区分服务器打盹的文案
    ...
    public function get_topics($gid,$page_size,$essence_type,$page_num,$parse_components=false) {
        if( empty($gid) ) {
            ...
            $this->assign( 'images', '/images/public/500.png' );
            $this->display("Public:error_code");
            exit;
        }
...
//            $v['text'] = $this->string_parse_face( $v['text'],22 );
  ...
  * */
    public function detail() {
        ...
        //相关话题
        $rela_data = $this->relatedTopics($tid);
        $this->assign('rela_data',$rela_data); //推荐话题
        ...
        //seo需要有圈子对应的wap地址
        $this->assign( 'mobile_url','//'. C('WAP_CIRCLE_URL').'topic-'.$tid.'.html');
     ...
     * 获取相关话题
     */
    private function relatedTopics($tid){
        $param = $data = [];
        $param['boxType'] = 1; //pc传1
        $param['topicId'] = $tid;  //暂时写死,调试用
        if(empty($param['topicId'])){
            return [];
        }
        //print_r($param);die;
        $result = $this->topic->getData($this->topic->relatedTopics,$param);
        if(!empty($result['data']['relatedTopics'])){
            $data = $result['data']['relatedTopics'];
        }
        return $data;
    }
    ...
    public function get_detail( $tid ) {
        ...
        if( !$data_detail ) {
            ...
            if($data_detail['code'] == 200){
                ...
                if($is_item == false || $this->userInfo['loginStatus'] != 3){
        ...
        if(!empty($data_detail['data'])){
            ...
            $praise_infos = $data_detail['data']['like'];
            if( !empty($praise_infos) ) {
                $data_detail['data']['praise_infos'] = $praise_infos;
                $data_detail['data']['praise_infos']['is_status'] = (isset($praise_infos['isLike']) && ( $praise_infos['isLike'] ) ) ? 0 : 1 ;
            }
        }
...
EOF;
                        ...
                        $v['text'] = nl2br($v['text']);
                        $img_str = '<img src="'.$default_img_url.'" alt="'.$temp_name.'" data-original="'.$v['url'].'" width="'.$width.'px" height="'.$height.'px"/>';
                        if(isset($v['scheme'])){
                            $img_str_tmp = '<a href="'.$v['scheme'].'" target="_blank">'.$img_str.'</a>>';
                            $img_str = $img_str_tmp;
                        }
...
EOF;
                ...
                if( $v['type'] == 'item' && ( !isset($v['richText'] ) || (isset($v['richText'] ) && $v['richText'])) ) {
                    ...
                    if($v['item']['itemId']){
                        //是否属团购
                        if($v['item']['pricetype'] == 'TUANPRICE'){
                            $flag_html = '<div class="buy"><em class="icon-tuan"></em></div>';
                        }else{
                            $flag_html = '<div class="buy"><em class="icon-rob"></em></div>';
                        }


                        $v['item']['salePrice'] = (isset($v['item']['finalPrice'])) ? $v['item']['finalPrice'] : 0;
                    }else{
                    ...
                    $item_url = productDetailUrlGen($v['shopId'],$v['outProductId'],$v['skuId'], $v['kid']);
...
<div class="publish-item">
	...
	<div class="publish-cont">
		...
		<div class="pub-row" data-node="price">
			<span class="red">￥</span>
			<strong class="money-inf" data-id="{$v['outProductId']}" data-skuId="{$v['skuId']}">{$money}</strong>
		</div> 
		{$flag_html}
...
EOF;
                ...
                if( 'mshop' == $v['type'] && !empty($v['mshopId']) ) {
                    $this->mshop_flag = 1;
                    $this->mshop_url = mshopDetailUrlGen($v['mshopId']) . '?intcmp=sj-sa-'.$tid;
                }
            }
    ...
    /*
     * handleRichJson 处理richjson字段
     */
    private function handleRichJson($text){
        $html = '';
        foreach ($text['child'] as $key=>$val){
            $attr = '';
            //拼接属性
            if(!empty($val['attr'])){
                foreach ($val['attr'] as $kk=>$vv){
                    $attr .= " ".$kk."='".$vv."'";
                }
            }
            //处理文本
            ...
            if($val['type'] == 'element'){
                //处理单标签
                if(in_array($val['tag'],$this->singleTag)){
                    if($val['tag'] == 'img'){
                        if(isset($val['video-tag'])){
                            $html .= '<div  class="videoContainer"  style="height:423px" video-id="'.$val['attr']['video-id'].'"id="'.$val['attr']['id'].'"></div>';
                        }else{

                            $imgStr  = ' src="//js.meixincdn.com/m/pc/dist/images/public/img-error.png?'.C('JS_VERSION').'"';
                            $imgStr .= ' data-src="'.$val['attr']['proto'].$val['attr']['_src'].'" ';

                            $imgArr = explode('.',$val['attr']['_src']);
                            array_pop($imgArr);
                            $imgStr .= ' data-original="'.implode('.',$imgArr).'-600!80.jpg"';
                            $html .= "<" . $val['tag'] . $attr . $imgStr . " />";
                        }
                    }else{
                        $html .= "<" . $val['tag'] . $attr . " />";
                    }
                    ...
                    $open = "<" . $val['tag'] . $attr . ">";
                    $close = "</" . $val['tag'] . ">";
                    $child = $this->handleRichJson($val);
                    $html .= $open . $child . $close;
                }
            }
        }
        return $html;
    ...
    /*
     * 删除话题|2018-01-10美店v3上线后 此接口可废弃 删除
     * @param id
     * */
    public function del(){
        $arrReturn = array();
