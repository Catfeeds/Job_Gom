...
<!--     <if condition="$list.status eq 2" >
    ...
    <if condition="$list.status eq 2 || $list.status eq 3 || $list.status eq -12 || $list.status eq -5 ">
            <if condition="$list.orderType neq 6 && $list.orderType neq 7 ">
            <notempty name="list.hasLogistics">
            <a target="_blank" href="<{$i_domain}>order/orderParcels?orderId=<{$list.id}>" class="order-blank-btn order-orange-btn" >查看包裹</a>
            </notempty>
            <else />
            <a target="_blank" href="<{$i_domain}>order/orderParcels?orderId=<{$list.id}>" class="order-blank-btn order-orange-btn">查看包裹</a>
            </if>
      ...
      <table data-node="orderList" class="shop-car-list order-information">
        <tr>
          <td colspan="6" class="align-left">
            <div class="check-all">
              <div class="shop-arl-check icon-fontsize"><em class="iconn-64"></em></div>
      			  <a target="_blank" href="<{$list.shop.id|shopDetailUrlGen}>" class="store-name-text"><{$list.shop.name}></a>
      			  <!--国美在线不显示联系商家-->
      			<if condition="$list.orderType neq 6 && $list.orderType neq 7">
      			  <a  href="<?=imUrl('shop',$list['shop']['id'])?>" target="_blank" data-action="entryBtn" class="connect-shop"><p>联系商家</p></a>
				<else />
              <a  href="javascript:;"  class="connect-shop gm-gray">联系商家</a>
				</if>
            </div>
          </td>
        </tr>
		...
		<tr>
          <td colspan="4">
		  <volist name="list.orderItems" id="item_sku" key="sku_key" >
            <div   class="products-by <?php if( $sku_key != $list['orderItemsCount'] ){ echo 'borb';}?>" >
				<div class="graphic width545 clearfix">
					<a  href="<{$item_sku.sku.item.id|productDetailUrlGen=$item_sku['mshop']['id'],###}>?skuid=<{$item_sku.sku.id}>" target="_blank">
					  <div class="product-bx"><img onerror="imgError(this)" src="<{$item_sku.sku.image|getResizeImg=###,80,80}>"></div>
					  ...
					  <a target="_blank" href="<{$item_sku.sku.item.id|productDetailUrlGen=$item_sku['mshop']['id'],###}>?skuid=<{$item_sku.sku.id}>" class="product-title"><{$item_sku.sku.item.shortName}></a>
						...
						<if condition="$item_sku['mshop']['id'] neq $list['shop']['id']">
						<p class="product-source">来自美店：<span><{$item_sku.mshop.name}></span></p>
						</if>
					  </div>
					 </a>
				</div>
				<span class="price-count">￥<{$item_sku.sku.price}></span><span class="price-count"><{$item_sku.quantity}></span>
				<div class="return-btn">
					<notempty name="item_sku.afterSalesFlag">
					<a href="<{$i_domain}>order/showDataInfo?orderid=<{$orderId}>&skuid=<{$item_sku.sku.id}>&goodid=<{$item_sku.sku.item.id}>" >申请售后</a>
					</notempty>
				</div>
            </div>
			</volist>
          </td>
          <td class="bor-r td-middle">
            <if condition="$list.platformCouponMoney neq '0.00'">
            <div class="new-border-bom">平台优惠券:￥<{$list.platformCouponMoney}></div>
			<elseif condition="$list.shopCouponMoney neq '0.00'"/>
            <div class="new-border-bom">店铺优惠券:￥<{$list.shopCouponMoney}></div>
			<else />
			<div class="new-border-bom">￥0.00</div>
			</if>
          </td>
          <td class="td-middle">
            <div class="new-border-bom">￥<{$list.shippingCost}></div>
          </td>
        </tr>
      </table>
	  <else />
      ...
      <table data-node="orderList" class="shop-car-list order-information">
        <tr>
          <td colspan="5" class="align-left">
            <div class="check-all">
              <div class="shop-arl-check icon-fontsize"><em class="iconn-64"></em></div>
			    <a href="<{$list.shop.id|shopDetailUrlGen}>" class="store-name-text" target="_blank"><{$list.shop.name}></a>
			       <!--国美在线不显示联系商家-->
				
      			<if condition="$list.orderType neq 6  &&  $list.orderType neq 7">
					<a data-action="ContactMerchant" href="javascript:;" class="connect-shop">联系商家</a>
				<else />
					<a  href="javascript:;" class="connect-shop gm-gray">联系商家</a>
      			</if>
            </div>
		   ...
		   <volist name="list.orderItems" id="item_sku" key="sku_key" >
            <div   class="products-by <?php if( $sku_key != $list['orderItemsCount'] ){ echo 'borb';}?>">
                ...
                <a href="<{$item_sku.sku.item.id|productDetailUrlGen=$item_sku['mshop']['id'],###}>?skuid=<{$item_sku.sku.id}>" target="_blank">
                  ...
                  <div class="product-name"><a target="_blank" href="<{$item_sku.sku.item.id|productDetailUrlGen=$item_sku['mshop']['id'],###}>?skuid=<{$item_sku.sku.id}>" class="product-title"><{$item_sku.sku.item.shortName}></a>
        					<notempty name="item_sku.sku.attributes">
        					<p class="product-color">
        					<volist name="item_sku.sku.attributes" id="item_attributes" key="attributes_key" >
        					<{$item_attributes.name}>：<{$item_attributes.value}> <?php if($attributes_key != count($item_sku['sku']['attributes'])):?>&emsp;<?php endif;?>
