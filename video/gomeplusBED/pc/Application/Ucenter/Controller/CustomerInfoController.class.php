...
class CustomerInfoController extends AuthController
{
	...
	public function getBackData(){
		...
		if($arrList['data'])
		{
			...
			foreach ($arrList['data']['afterSalesOrders'] as $key => $Item)
			{
				...
				$arr['orderType']  = $Item['orderType'];//订单状态6,7在线商品
				$arr['collectingMethod'] = isset($Item['collectingMethod']) ? $Item['collectingMethod'] : "" ;//寄回方式
				$strShopName = '';
			    $arrName = array();
				foreach ($Item['orderItems'] as &$Info) 
				{
					...
					$skuid =$Info['sku']['id'];
					if(mb_strlen($name) > 8 ){//产品要求大于24个字
	...
	public function showCustomerInfo(){
		...
		if($arrInfo['success']){
			...
			$moneyTotal = 0;
			$arrLineGoods = array(
						'collectingMethod' => $arrInfo['data']['collectingMethod'],//送货方式
						'mailAddress'      => $arrInfo['data']['mailAddress'],//快递至买家地址
					);
			$orderType = empty($arrInfo['data']['orderType']) ? 0 : intval($arrInfo['data']['orderType']);//订单类型6,7在线订单
			$status    = $arrInfo['data']['status'];//订单状态
			...
			$this->assign('linegoods',$arrLineGoods);
	...
	public function getOrderLogistics(){
		...
		/*
		$list = array();
	    $param['orderId'] = I('param.id',0,'intval');//售后的id
	    //物流信息
	    $param['type'] = 2;//查看物流类型（1：查看正向订单，2：查看逆向订单）
	    $param['shopType'] = 1 ;//店铺类型 1：XPOP店 2:员工达人店
