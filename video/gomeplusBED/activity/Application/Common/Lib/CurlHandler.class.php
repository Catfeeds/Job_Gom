...
class CurlHandler {
	...
	public function request($url, $param, $method='get') {
		...
		$gome_in = false;
		$domain = [
			'gome.com.cn','uatplus.com','access.ec.api','accessuat.ec.api','atguat.com.cn',
			'api-v.gomeplus.com','gvsout.pre.video.api'
		];
		foreach( $domain as $iv ) {
		...
		if( isset( $data['error'] ) && $data['error'] ){
            return output($data['code'],$data['message'],$data['data'],$data['error'] ) ;
        }
	...
	private function parseResponse($log) {
		...
		else
		{
			...
			if(!isset($data['code'])){
                ...
                if( isset( $data['error'] ) ){
                    $format_data['error'] = $data['error'];
                }
			}elseif($data['code'] === 0){
				$format_data['code'] = 200;
			}else {
				$format_data['code'] = $data['code'];
				$format_data['message'] = $data['message'];
			}
		}
