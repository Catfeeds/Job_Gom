...
import isSecretBrowse from 'util/isSecretBrowse.js';
...
class Base extends EventBus {
	constructor(opts = {}) {
		...
		this.localStorageKey = 'gmv_m_action';
	...
	_init() {
		...
		function doEvent(e) {
			...
			if (!loginFlag) {
				isSecretBrowse() || localStorage.setItem(_this.localStorageKey, JSON.stringify({ action: action, id: id, status:status}));
				login();
				return false;
			}
			cache[key] = 1;
	...
	request(opts) {
		...
		fetch.post(this.api, {
			...
			success: function(data) {
				...
				// 登录过期
				if (code == 200001) {
					isSecretBrowse() || localStorage.setItem(_this.localStorageKey, JSON.stringify({ action: action, id: id, status:actionType^1}));
					location.href = page.loginUrl;
					return false;
				}
				if (code == 200 || code == 400008) {
